...
div[data-testid="stTabs"] [data-baseweb="tab-list"] { border-bottom: 1px solid rgba(201,160,102,.15) !important; background: transparent !important; gap: 0 !important; }
...
# ── HTTP SESSION — headers realistas para evitar 403 ──────────────────────────
USER_AGENTS = [
...
def make_session() -> requests.Session:
    ...
    # Visitar primero la home para obtener cookies reales
    try:
        s.get("https://www.fragrantica.com/", timeout=10)
...
def search_fragrantica(query: str) -> dict:
    ...
    if not perf_url:
        # Intentar búsqueda alternativa via suggest API
        suggest_url = f"https://www.fragrantica.com/ajax/suggest.php?q={requests.utils.quote(query)}"
        rs = session.get(suggest_url, timeout=10)
        if rs.ok:
            slugs = re.findall(r'href="(/perfume/[^"]+\d+\.html)"', rs.text)
            if slugs:
                perf_url = "https://www.fragrantica.com" + slugs[0]
    if not perf_url:
        raise ValueError(f'No se encontró "{query}" en Fragrantica')

    # 3 — Cargar ficha del perfume
...
def parse_page(html: str, url: str) -> dict:
    ...
    if not desc:
        ...
        desc = (meta_desc.get("content", "")[:300]) if meta_desc else ""
    ...
    season = [s for s, p in [("🌸 Primavera","spring"),("☀️ Verano","summer"),
                               ("🍂 Otoño","fall|autumn"),("❄️ Invierno","winter")]
              if re.search(p, html, re.I)]
    if not season: season = ["🌍 Todo el año"]

    time_day = [t for t, p in [("🌤 Diurno","day\\s*time|daytime"),("🌙 Nocturno","night|evening")]
                if re.search(p, html, re.I)]
...
def extract_notes(soup, html, note_type):
    names = []
    heads = {
        "top":    re.compile(r"Top Notes?", re.I),
    ...
    if not names:
        chunk_re = {
            "top":    r"Top Notes?[\s\S]{0,1500}?(?=Middle|Heart|Base)",
            "middle": r"(?:Middle|Heart) Notes?[\s\S]{0,1500}?(?=Base)",
            "base":   r"Base Notes?[\s\S]{0,1500}",
        }
    ...
    unique = list(dict.fromkeys(
        n[0].upper() + n[1:].lower() for n in names if len(n) > 1
    ))[:8]
...
with col1:
    query = st.text_input("Fragancia", placeholder="Nombre del perfume… (ej: Sauvage Dior)",
...
if search_btn and query:
    with st.spinner("Conectando con Fragrantica…"):
        try:
            p = search_fragrantica(query)
        except requests.exceptions.HTTPError as e:
            if "403" in str(e):
                st.error("⚠️ Fragrantica ha bloqueado temporalmente la petición.\n\nEspera unos segundos y vuelve a intentarlo. Si persiste, prueba con el nombre en inglés.")
            else:
                st.error(f"❌ Error de conexión: {e}")
            st.stop()
        except ValueError as e:
            st.error(f"🔍 {e}\n\nPrueba con: marca + nombre (ej: *Dior Sauvage*, *Chanel N5*, *YSL Black Opium*)")
            st.stop()
    ...
    with hero_col1:
        if p["img_url"]:
            ...
            st.markdown("<div style='font-size:60px;text-align:center;opacity:.2;padding:20px 0'>🫙</div>",
                        unsafe_allow_html=True)
    ...
    with hero_col2:
        ...
        if p["desc"]:
            st.markdown(f"<p style='font-size:13.5px;line-height:1.75;color:rgba(237,227,208,.6)'>{p['desc']}</p>",
                        unsafe_allow_html=True)
